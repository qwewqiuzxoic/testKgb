...
function EduNoticeBoard() {
  if(user===null){
    ...
    const openModal = () => {
        dispatch(boardPostLoginInput(user,"교육공지","INS"))
        setModalOpen(true);
    }
    const closeModal = () => {
        setModalOpen(false);
    ...
    const callback = () =>{
        dispatch(getBoardList(user.brand,"교육공지",1))
        pageCount.current = 1;
        closeModal();
      }
      const onsubmit= ()=>{
    ...
    useEffect(() => {
        dispatch(getBoardList(user.brand,"교육공지",pageCount.current))
        dispatch(getBoardTopList(user.brand,"교육공지"))
    ...
    const infiniteScroll = () => {
      ...
      if (scrollTop + clientHeight >= scrollHeight) {
        pageCount.current += 1;
        dispatch(getBoardList(user.brand,"소사장공지사항",pageCount.current))
  ...
  return (
    <Wrapper>
            <BoardTitle title="교육공지 "subtit="교육공지 게시판입니다" check={false}/>
            <ContentArea>
            <NoticeWrap>

            { list.map((item,index)=>{
              return (
                  <BoardList key={index} title={item.title} regdate={item.regdate} board_sn={item.board_sn} index={index} loginname={item.loginname} tname={item.tname} countview={item.countview} cnt={item.cnt} adu={item.adu} typeCheck={item.teamNm} classname="important" boardCode="6"/>
              )
            })} 
            </NoticeWrap>

            <BoardListWrap boardCode="6"/>
            </ContentArea>
