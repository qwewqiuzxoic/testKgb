...
function NoticeBoard() {
  if(user===null){
    ...
    const openModal = () => {
        dispatch(boardPostLoginInput(user,"소사장공지사항","INS"))
        setModalOpen(true);
    }
    const closeModal = () => {
        setModalOpen(false);
    ...
    const callback = () =>{
        dispatch(getBoardList(user.brand,"소사장공지사항",1))
        pageCount.current = 1;
        closeModal();
      }
      const onsubmit= ()=>{
    ...
    useEffect(() => {
        ...
        dispatch(getBoardTopList(user.brand,"소사장공지사항"))
  ...
  return (
    <Wrapper>
            <BoardTitle title="공지사항"subtit="공지사항 게시판입니다" check={false}/>
            <ContentArea>
            <NoticeWrap>
